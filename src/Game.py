...
level = 1 # difficulty level which determines the side lengths of the current maze
maze: list[list[int]] = generateMaze(2 * level + 3, 2 * level + 3)
...
difficultyToSideLength = lambda d : 2 * (d + 1) + 1
...
def setupNewMaze():
    ...
    maze = generateMaze(level * 2 + 3, level * 2 + 3)
    playerDirection = 0
    playerPosition = Vector2(1, 1)
    r3D.setMaze(maze)
...
def movePlayer(dx: int, dy: int) -> Vector2:
    ...
    global mazeWidth, mazeHeight, level
